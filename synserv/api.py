...
from service import syncwserv_rpc, syncwserv_threaded_rpc, ccnet_threaded_rpc
from pyrpcsyncwerk import RpcsyncwerkError
import json

"""
General rules for return values and exception handling of Syncwerk python API:
- Read operations return corresponding values. Raises exceptions on parameter errors
  or I/O errors in syncwerk-server-daemon.
...
class SyncwerkAPI(object):
    ...
    def get_fileserver_access_token(self, repo_id, obj_id, op, username, use_onetime=True):
        ...
        return syncwserv_rpc.web_get_access_token(repo_id, obj_id, op, username,
                                                 onetime)
    ...
    def query_fileserver_access_token(self, token):
        ...
        """
        return syncwserv_rpc.web_query_access_token(token)
    ...
    def query_zip_progress(self, token):
        ...
        """
        return syncwserv_rpc.query_zip_progress(token)

    def cancel_zip_task(self, token):
        return syncwserv_rpc.cancel_zip_task(token)
    ...
    def is_password_set(self, repo_id, username):
        ...
        """
        return syncwserv_rpc.is_passwd_set(repo_id, username)

    def get_decrypt_key(self, repo_id, username):
        """
        Return: a CryptKey object (lib/crypt.vala)
        """
        return syncwserv_rpc.get_decrypt_key(repo_id, username)

    def change_repo_passwd(self, repo_id, old_passwd, new_passwd, user):
        return syncwserv_threaded_rpc.change_repo_passwd(repo_id, old_passwd,
                                                        new_passwd, user)
    def check_passwd(self, repo_id, magic):
        return syncwserv_threaded_rpc.check_passwd(repo_id, magic)

    def set_passwd(self, repo_id, user, passwd):
        return syncwserv_threaded_rpc.set_passwd(repo_id, user, passwd)

    def unset_passwd(self, repo_id, user, passwd):
        return syncwserv_threaded_rpc.unset_passwd(repo_id, user, passwd)

    def generate_magic_and_random_key(self, enc_version, repo_id, password):
        return syncwserv_threaded_rpc.generate_magic_and_random_key(enc_version, repo_id, password)

    # repo manipulation

    def create_repo(self, name, desc, username, passwd):
        return syncwserv_threaded_rpc.create_repo(name, desc, username, passwd)

    def create_enc_repo(self, repo_id, name, desc, username, magic, random_key, enc_version):
        return syncwserv_threaded_rpc.create_enc_repo(repo_id, name, desc, username, magic, random_key, enc_version)

    def get_repo(self, repo_id):
        """
        Return: a Repo object (lib/repo.vala)
        """
        return syncwserv_threaded_rpc.get_repo(repo_id)

    def remove_repo(self, repo_id):
        return syncwserv_threaded_rpc.remove_repo(repo_id)

    def get_repo_list(self, start, limit):
        """
        Return: a list of Repo objects (lib/repo.vala)
        """
        return syncwserv_threaded_rpc.get_repo_list(start, limit)

    def count_repos(self):
        return syncwserv_threaded_rpc.count_repos()

    def edit_repo(self, repo_id, name, description, username):
        return syncwserv_threaded_rpc.edit_repo(repo_id, name, description, username)

    def is_repo_owner(self, username, repo_id):
        """
        Return 1 if True, otherwise 0.
        """
        return syncwserv_threaded_rpc.is_repo_owner(username, repo_id)

    def set_repo_owner(self, email, repo_id):
        return syncwserv_threaded_rpc.set_repo_owner(email, repo_id)

    def get_repo_owner(self, repo_id):
        """
        Return: repo owner in string
        """
        return syncwserv_threaded_rpc.get_repo_owner(repo_id)

    def get_owned_repo_list(self, username, ret_corrupted=False, start=-1, limit=-1):
        """
        Return: a list of Repo objects
        """
        return syncwserv_threaded_rpc.list_owned_repos(username,
                                                      1 if ret_corrupted else 0,
                                                      start, limit)

    def get_orphan_repo_list(self):
        return syncwserv_threaded_rpc.get_orphan_repo_list()

    def get_repo_size(self, repo_id):
        return syncwserv_threaded_rpc.server_repo_size(repo_id)

    def revert_repo(self, repo_id, commit_id, username):
        return syncwserv_threaded_rpc.revert_on_server(repo_id, commit_id, username)

    def diff_commits(self, repo_id, old_commit, new_commit, fold_dir_diff = 1):
        """
        Return: a list of DiffEntry objects (lib/repo.vala)
        """
        return syncwserv_threaded_rpc.get_diff(repo_id, old_commit, new_commit, fold_dir_diff)

    def get_commit_list(self, repo_id, offset, limit):
        """
        Return: a list of Commit objects (lib/commit.vala)
        """
        return syncwserv_threaded_rpc.get_commit_list(repo_id, offset, limit)

    def get_commit(self, repo_id, repo_version, cmt_id):
        """ Get a commit. """
        try:
            ret = syncwserv_threaded_rpc.get_commit(repo_id, repo_version, cmt_id)
        except RpcsyncwerkError:
            ret = None
        return ret

    def get_system_default_repo_id (self):
        return syncwserv_threaded_rpc.get_system_default_repo_id()

    def get_org_id_by_repo_id (self, repo_id):
        return syncwserv_threaded_rpc.get_org_id_by_repo_id(repo_id)
    ...
    def is_valid_filename(self, repo_id, filename):
        ...
        """
        return syncwserv_threaded_rpc.is_valid_filename(repo_id, filename)

    def get_file_size(self, store_id, version, file_id):
        return syncwserv_threaded_rpc.get_file_size(store_id, version, file_id)
    ...
    def get_dir_size(self, store_id, version, dir_id):
        ...
        """
        return syncwserv_threaded_rpc.get_dir_size(store_id, version, dir_id)

    def get_file_id_by_path(self, repo_id, path):
        """
        Returns None if path not found. Only raise exception on parameter or IO error.
        """
        return syncwserv_threaded_rpc.get_file_id_by_path(repo_id, path)

    def get_file_id_by_commit_and_path(self, repo_id, commit_id, path):
        return syncwserv_threaded_rpc.get_file_id_by_commit_and_path(repo_id,
                                                                    commit_id,
                                                                    path)
    ...
    def get_dirent_by_path(self, repo_id, path):
        ...
        """
        return syncwserv_threaded_rpc.get_dirent_by_path(repo_id, path)

    def list_file_by_file_id(self, repo_id, file_id, offset=-1, limit=-1):
        # deprecated, use list_blocks_by_file_id instead.
        return syncwserv_threaded_rpc.list_file_blocks(repo_id, file_id, offset, limit)
    ...
    def get_dir_id_by_path(self, repo_id, path):
        return syncwserv_threaded_rpc.get_dir_id_by_path(repo_id, path)
    ...
    def list_dir_by_dir_id(self, repo_id, dir_id, offset=-1, limit=-1):
        ...
        """
        return syncwserv_threaded_rpc.list_dir(repo_id, dir_id, offset, limit)

    def list_dir_by_path(self, repo_id, path, offset=-1, limit=-1):
        dir_id = syncwserv_threaded_rpc.get_dir_id_by_path(repo_id, path)
        if dir_id is None:
            return None
        return syncwserv_threaded_rpc.list_dir(repo_id, dir_id, offset, limit)
    ...
    def list_dir_by_commit_and_path(self, repo_id,
        ...
        dir_id = syncwserv_threaded_rpc.get_dir_id_by_commit_and_path(repo_id, commit_id, path)
        if dir_id is None:
            return None
        return syncwserv_threaded_rpc.list_dir(repo_id, dir_id, offset, limit)
    
    def get_dir_id_by_commit_and_path(self, repo_id, commit_id, path):
        return syncwserv_threaded_rpc.get_dir_id_by_commit_and_path(repo_id, commit_id, path)

    def list_dir_with_perm(self, repo_id, dir_path, dir_id, user, offset=-1, limit=-1):
        return syncwserv_threaded_rpc.list_dir_with_perm (repo_id, dir_path, dir_id, user, offset, limit)

    def mkdir_with_parents (self, repo_id, parent_dir, relative_path, username):
        return syncwserv_threaded_rpc.mkdir_with_parents(repo_id, parent_dir, relative_path, username)

    def get_file_count_info_by_path(self, repo_id, path):
        return syncwserv_threaded_rpc.get_file_count_info_by_path(repo_id, path)

    def get_total_storage (self):
        return syncwserv_threaded_rpc.get_total_storage()

    def get_total_file_number (self):
        return syncwserv_threaded_rpc.get_total_file_number()

    # file/dir operations

    def post_file(self, repo_id, tmp_file_path, parent_dir, filename, username):
        """Add a file to a directory"""
        return syncwserv_threaded_rpc.post_file(repo_id, tmp_file_path, parent_dir,
                                               filename, username)

    def post_empty_file(self, repo_id, parent_dir, filename, username):
        return syncwserv_threaded_rpc.post_empty_file(repo_id, parent_dir,
                                                     filename, username)
    ...
    def put_file(self, repo_id, tmp_file_path, parent_dir, filename,
        ...
        """
        return syncwserv_threaded_rpc.put_file(repo_id, tmp_file_path, parent_dir,
    ...
    def del_file(self, repo_id, parent_dir, filename, username):
        return syncwserv_threaded_rpc.del_file(repo_id, parent_dir, filename, username)
    ...
    def copy_file(self, src_repo, src_dir, src_filename, dst_repo,
        ...
        return syncwserv_threaded_rpc.copy_file(src_repo, src_dir, src_filename,
    ...
    def move_file(self, src_repo, src_dir, src_filename, dst_repo, dst_dir,
        ...
        return syncwserv_threaded_rpc.move_file(src_repo, src_dir, src_filename,
                                               dst_repo, dst_dir, dst_filename,
                                               replace, username, need_progress, synchronous)

    def get_copy_task(self, task_id):
        return syncwserv_rpc.get_copy_task(task_id)

    def cancel_copy_task(self, task_id):
        return syncwserv_rpc.cancel_copy_task(task_id)

    def rename_file(self, repo_id, parent_dir, oldname, newname, username):
        return syncwserv_threaded_rpc.rename_file(repo_id, parent_dir,
                                                 oldname, newname, username)

    def post_dir(self, repo_id, parent_dir, dirname, username):
        """Add a directory"""
        return syncwserv_threaded_rpc.post_dir(repo_id, parent_dir, dirname, username)

    def revert_file(self, repo_id, commit_id, path, username):
        return syncwserv_threaded_rpc.revert_file(repo_id, commit_id, path, username)

    def revert_dir(self, repo_id, commit_id, path, username):
        return syncwserv_threaded_rpc.revert_dir(repo_id, commit_id, path, username)
    ...
    def get_deleted(self, repo_id, show_days, path='/', scan_stat=None, limit=100):
        ...
        None.
        """
        return syncwserv_threaded_rpc.get_deleted(repo_id, show_days, path, scan_stat, limit)
    ...
    def get_file_revisions(self, repo_id, commit_id, path, limit):
        ...
        """
        return syncwserv_threaded_rpc.list_file_revisions(repo_id, commit_id, path, limit)
    ...
    def get_files_last_modified(self, repo_id, parent_dir, limit):
        ...
        """
        return syncwserv_threaded_rpc.calc_files_last_modified(repo_id,
                                                              parent_dir, limit)
    ...
    def get_repo_history_limit(self, repo_id):
        ...
        """
        return syncwserv_threaded_rpc.get_repo_history_limit(repo_id)

    def set_repo_history_limit(self, repo_id, days):
        """
        Set repo history limit in days. Pass -1 if set to unlimited.
        """
        return syncwserv_threaded_rpc.set_repo_history_limit(repo_id, days)

    def check_repo_blocks_missing(self, repo_id, blklist):
        return syncwserv_threaded_rpc.check_repo_blocks_missing(repo_id, blklist)
    ...
    def share_repo(self, repo_id, from_username, to_username, permission):
        return syncwserv_threaded_rpc.add_share(repo_id, from_username,
                                               to_username, permission)

    def remove_share(self, repo_id, from_username, to_username):
        return syncwserv_threaded_rpc.remove_share(repo_id, from_username,
                                                  to_username)
    
    def set_share_permission(self, repo_id, from_username, to_username, permission):
        return syncwserv_threaded_rpc.set_share_permission(repo_id, from_username,
                                                          to_username, permission)

    def share_subdir_to_user(self, repo_id, path, owner, share_user, permission, passwd=''):
        return syncwserv_threaded_rpc.share_subdir_to_user(repo_id, path, owner,
                                                          share_user, permission, passwd)

    def unshare_subdir_for_user(self, repo_id, path, owner, share_user):
        return syncwserv_threaded_rpc.unshare_subdir_for_user(repo_id, path, owner,
                                                             share_user)

    def update_share_subdir_perm_for_user(self, repo_id, path, owner,
                                          share_user, permission):
        return syncwserv_threaded_rpc.update_share_subdir_perm_for_user(repo_id, path, owner,
                                                                       share_user, permission)
    ...
    def get_shared_repo_by_path(self, repo_id, path, shared_to, is_org=False):
        ...
        """
        return syncwserv_threaded_rpc.get_shared_repo_by_path(repo_id, path, shared_to, 1 if is_org else 0)
    ...
    def get_share_out_repo_list(self, username, start, limit):
        ...
        Return: a list of Repo objects
        """
        return syncwserv_threaded_rpc.list_share_repos(username, "from_email",
                                                      start, limit)
    ...
    def get_share_in_repo_list(self, username, start, limit):
        ...
        """
        return syncwserv_threaded_rpc.list_share_repos(username, "to_email",
                                                      start, limit)
    ...
    def list_repo_shared_to(self, from_user, repo_id):
        ...
        """
        return syncwserv_threaded_rpc.list_repo_shared_to(from_user, repo_id)

    def repo_has_been_shared(self, repo_id, including_groups=False):
        return True if syncwserv_threaded_rpc.repo_has_been_shared(repo_id, 1 if including_groups else 0) else False

    # share repo to group
    def group_share_repo(self, repo_id, group_id, username, permission):
        # deprecated, use ``set_group_repo``
        return syncwserv_threaded_rpc.group_share_repo(repo_id, group_id,
                                                      username, permission)
    ...
    def group_unshare_repo(self, repo_id, group_id, username):
        ...
        return syncwserv_threaded_rpc.group_unshare_repo(repo_id, group_id, username)

    def unset_group_repo(self, repo_id, group_id, username):
        return syncwserv_threaded_rpc.group_unshare_repo(repo_id, group_id, username)

    def get_shared_group_ids_by_repo(self, repo_id):
        group_ids = syncwserv_threaded_rpc.get_shared_groups_by_repo(repo_id)
    ...
    def list_repo_shared_group(self, from_user, repo_id):
        ...
        return syncwserv_threaded_rpc.list_repo_shared_group(from_user, repo_id)
    ...
    def get_group_shared_repo_by_path (self, repo_id, path, group_id, is_org=False):
        ...
        otherwise, 'repo_id' represents for the origin repo, return virtual repo
        """
        return syncwserv_threaded_rpc.get_group_shared_repo_by_path(repo_id, path, group_id, 1 if is_org else 0)

    def get_group_repos_by_user (self, user):
        """
        Return all the repos in all groups that the @user belongs to.
        """
        return syncwserv_threaded_rpc.get_group_repos_by_user(user)

    def get_org_group_repos_by_user (self, user, org_id):
        return syncwserv_threaded_rpc.get_org_group_repos_by_user(user, org_id)
    ...
    def share_subdir_to_group(self, repo_id, path, owner, share_group, permission, passwd=''):
        return syncwserv_threaded_rpc.share_subdir_to_group(repo_id, path, owner,
                                                           share_group, permission, passwd)

    def unshare_subdir_for_group(self, repo_id, path, owner, share_group):
        return syncwserv_threaded_rpc.unshare_subdir_for_group(repo_id, path, owner,
                                                              share_group)

    def update_share_subdir_perm_for_group(self, repo_id, path, owner,
                                           share_group, permission):
        return syncwserv_threaded_rpc.update_share_subdir_perm_for_group(repo_id, path, owner,
                                                                        share_group, permission)
    ...
    def get_group_repoids(self, group_id):
        ...
        """
        repo_ids = syncwserv_threaded_rpc.get_group_repoids(group_id)
        if not repo_ids:
            return []
        l = []
    ...
    def get_repos_by_group(self, group_id):
        ...
        Return: a list of Repo objects
        """
        return syncwserv_threaded_rpc.get_repos_by_group(group_id)
    ...
    def get_group_repos_by_owner(self, username):
        ...
        Return: a list of Repo objects
        """
        return syncwserv_threaded_rpc.get_group_repos_by_owner(username)

    def remove_group_repos_by_owner(self, group_id, username):
        """
        Unshare all repos a user shared to a group.
        """
        return syncwserv_threaded_rpc.remove_repo_group(group_id, username)
    ...
    def remove_group_repos(self, group_id):
        ...
        """
        return syncwserv_threaded_rpc.remove_repo_group(group_id, None)

    def set_group_repo_permission(self, group_id, repo_id, permission):
        return syncwserv_threaded_rpc.set_group_repo_permission(group_id, repo_id,
                                                               permission)
    ...
    def get_shared_users_for_subdir(self, repo_id, path, from_user):
        ...
        """
        return syncwserv_threaded_rpc.get_shared_users_for_subdir(repo_id, path, from_user)
    ...
    def get_shared_groups_for_subdir(self, repo_id, path, from_user):
        ...
        """
        return syncwserv_threaded_rpc.get_shared_groups_for_subdir(repo_id, path, from_user)

    def get_shared_users_by_repo(self, repo_id):
        users = []
        # get users that the repo is shared to
        shared_users = syncwserv_threaded_rpc.get_shared_users_by_repo (repo_id)
    ...
    def add_inner_pub_repo(self, repo_id, permission):
        return syncwserv_threaded_rpc.set_inner_pub_repo(repo_id, permission)

    def remove_inner_pub_repo(self, repo_id):
        return syncwserv_threaded_rpc.unset_inner_pub_repo(repo_id)

    def get_inner_pub_repo_list(self):
        """
        Return: a list of Repo objects.
        """
        return syncwserv_threaded_rpc.list_inner_pub_repos()

    def list_inner_pub_repos_by_owner(self, repo_owner):
        """
        Return: a list of Repo objects.
        """
        return syncwserv_threaded_rpc.list_inner_pub_repos_by_owner(repo_owner)

    def count_inner_pub_repos(self):
        return syncwserv_threaded_rpc.count_inner_pub_repos()

    def is_inner_pub_repo(self, repo_id):
        return syncwserv_threaded_rpc.is_inner_pub_repo(repo_id)
    ...
    def check_permission(self, repo_id, user):
        ...
        Return: 'r', 'rw', or None
        """
        return syncwserv_threaded_rpc.check_permission(repo_id, user)
    ...
    def check_permission_by_path(self, repo_id, path, user):
        ...
        Return: 'r', 'rw', or None
        """
        return syncwserv_threaded_rpc.check_permission_by_path(repo_id, path, user)

    # token
    def generate_repo_token(self, repo_id, username):
        """Generate a token for sync a repo
        """
        return syncwserv_threaded_rpc.generate_repo_token(repo_id, username)

    def delete_repo_token(self, repo_id, token, user):
        return syncwserv_threaded_rpc.delete_repo_token(repo_id, token, user)

    def list_repo_tokens(self, repo_id):
        """
        Return: a list of RepoTokenInfo objects.
        """
        return syncwserv_threaded_rpc.list_repo_tokens(repo_id)

    def list_repo_tokens_by_email(self, username):
        return syncwserv_threaded_rpc.list_repo_tokens_by_email(username)

    def delete_repo_tokens_by_peer_id(self, email, peer_id):
        return syncwserv_threaded_rpc.delete_repo_tokens_by_peer_id(email, peer_id)

    def delete_repo_tokens_by_email(self, email):
        return syncwserv_threaded_rpc.delete_repo_tokens_by_email(email)

    # quota
    def get_user_self_usage(self, username):
        """Get the sum of repos' size of the user"""
        return syncwserv_threaded_rpc.get_user_quota_usage(username)

    def get_user_share_usage(self, username):
        # sum (repo_size * number_of_shares)
        return syncwserv_threaded_rpc.get_user_share_usage(username)

    def get_user_quota(self, username):
        """
        Return: -2 if quota is unlimited; otherwise it must be number > 0.
        """
        return syncwserv_threaded_rpc.get_user_quota(username)

    def set_user_quota(self, username, quota):
        return syncwserv_threaded_rpc.set_user_quota(username, quota)
    ...
    def check_quota(self, repo_id, delta=0):
        return syncwserv_threaded_rpc.check_quota(repo_id, delta)

    # virtual repo
    def create_virtual_repo(self, origin_repo_id, path, repo_name, repo_desc, owner, passwd=''):
        return syncwserv_threaded_rpc.create_virtual_repo(origin_repo_id,
                                                         path,
                                                         repo_name,
                                                         repo_desc,
                                                         owner,
    ...
    def get_virtual_repos_by_owner(self, owner):
        return syncwserv_threaded_rpc.get_virtual_repos_by_owner(owner)

    def get_virtual_repo(self, origin_repo, path, owner):
        return syncwserv_threaded_rpc.get_virtual_repo(origin_repo, path, owner)

    # Clean trash

    def clean_up_repo_history(self, repo_id, keep_days):
        return syncwserv_threaded_rpc.clean_up_repo_history(repo_id, keep_days)

    # Trashed repos
    def get_trash_repo_list(self, start, limit):
        return syncwserv_threaded_rpc.get_trash_repo_list(start, limit)

    def del_repo_from_trash(self, repo_id):
        return syncwserv_threaded_rpc.del_repo_from_trash(repo_id)

    def restore_repo_from_trash(self, repo_id):
        return syncwserv_threaded_rpc.restore_repo_from_trash(repo_id)

    def get_trash_repos_by_owner(self, owner):
        return syncwserv_threaded_rpc.get_trash_repos_by_owner(owner)

    def get_trash_repo_owner (self, repo_id):
        return syncwserv_threaded_rpc.get_trash_repo_owner(repo_id)

    def empty_repo_trash(self):
        return syncwserv_threaded_rpc.empty_repo_trash()

    def empty_repo_trash_by_owner(self, owner):
        return syncwserv_threaded_rpc.empty_repo_trash_by_owner(owner)

    # Server config
    def get_server_config_int (self, group, key):
        return syncwserv_threaded_rpc.get_server_config_int (group, key)

    def set_server_config_int (self, group, key, value):
        return syncwserv_threaded_rpc.set_server_config_int (group, key, value)

    def get_server_config_int64 (self, group, key):
        return syncwserv_threaded_rpc.get_server_config_int64 (group, key)

    def set_server_config_int64 (self, group, key, value):
        return syncwserv_threaded_rpc.set_server_config_int64 (group, key, value)

    def get_server_config_string (self, group, key):
        return syncwserv_threaded_rpc.get_server_config_string (group, key)

    def set_server_config_string (self, group, key, value):
        return syncwserv_threaded_rpc.set_server_config_string (group, key, value)

    def get_server_config_boolean (self, group, key):
        return bool(syncwserv_threaded_rpc.get_server_config_boolean (group, key))

    def set_server_config_boolean (self, group, key, value):
        i_value = 1 if bool(value) else 0
        return syncwserv_threaded_rpc.set_server_config_boolean (group, key, i_value)

    def del_org_group_repo(self, repo_id, org_id, group_id):
        syncwserv_threaded_rpc.del_org_group_repo(repo_id, org_id, group_id)
    ...
    def list_org_inner_pub_repos(self, org_id):
        return syncwserv_threaded_rpc.list_org_inner_pub_repos(org_id)

    def convert_repo_path(self, repo_id, path, user, is_org=False):
        return syncwserv_threaded_rpc.convert_repo_path(repo_id, path, user, 1 if is_org else 0)

syncwerk_api = SyncwerkAPI()
