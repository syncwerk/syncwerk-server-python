...
_DEBUG = 'SYNCWERK_DEBUG' in os.environ

ENVIRONMENT_VARIABLES = ('CCNET_CONF_DIR', 'SYNCWERK_CONF_DIR')
...
def _load_path_from_env(key, check=True):
    ...
    if not v:
        if check:
            raise ImportError("Synserv cannot be imported, because environment variable %s is undefined." % key)
        return None
    if _DEBUG:
...
SYNCWERK_CENTRAL_CONF_DIR = _load_path_from_env('SYNCWERK_CENTRAL_CONF_DIR', check=False)

pool = ccnet.ClientPool(CCNET_CONF_PATH, central_config_dir=SYNCWERK_CENTRAL_CONF_DIR)
...
config.read(os.path.join(SYNCWERK_CENTRAL_CONF_DIR if SYNCWERK_CENTRAL_CONF_DIR else CCNET_CONF_PATH,
                         'ccnet.conf'))
...
config.read(os.path.join(SYNCWERK_CENTRAL_CONF_DIR if SYNCWERK_CENTRAL_CONF_DIR else SYNCWERK_CONF_DIR,
                         'server.conf'))
...
def get_repos():
    ...
    """
    return syncwserv_threaded_rpc.get_repo_list("", 100)
...
def edit_repo(repo_id, name, desc, user):
    try:
        ret = syncwserv_threaded_rpc.edit_repo(repo_id, name, desc, user)
    except RpcsyncwerkError, e:
        ret = -1
...
def create_repo(name, desc, user, passwd):
    ...
    """
    try:
        ret = syncwserv_threaded_rpc.create_repo(name, desc, user, passwd)
    except RpcsyncwerkError, e:
        logger.error(e)
        ret = None
    return ret
...
def remove_repo(repo_id):
    ...
    """
    try:
        ret = syncwserv_threaded_rpc.remove_repo(repo_id)
    except RpcsyncwerkError, e:
        logger.error(e)
        ret = -1
    return True if ret == 0 else False
...
def list_personal_repos_by_owner(owner):
    ...
    """
    try:
        repos = syncwserv_threaded_rpc.list_owned_repos(owner)
    except RpcsyncwerkError:
        repos = []
    return repos

def get_repo_token_nonnull(repo_id, username):
    return syncwserv_threaded_rpc.get_repo_token_nonnull (repo_id, username)
...
def get_repo_owner(repo_id):
    ...
    """
    try:
        ret = syncwserv_threaded_rpc.get_repo_owner(repo_id)
    except RpcsyncwerkError:
        ret = ''
    return ret
...
def is_repo_owner(user, repo_id):
    ...
    """
    try:
        ret = syncwserv_threaded_rpc.is_repo_owner(user, repo_id)
    except RpcsyncwerkError:
        ret = 0
    return ret
...
def server_repo_size(repo_id):
    try:
        size = syncwserv_threaded_rpc.server_repo_size(repo_id)
    except RpcsyncwerkError:
        size = 0
    return size
...
def create_org_repo(repo_name, repo_desc, user, passwd, org_id):
    ...
    """
    try:
        repo_id = syncwserv_threaded_rpc.create_org_repo(repo_name, repo_desc,
                                                        user, passwd, org_id)
    except RpcsyncwerkError:
        repo_id = None
...
def list_org_repos_by_owner(org_id, user):
    try:
        repos = syncwserv_threaded_rpc.list_org_repos_by_owner(org_id, user)
    except RpcsyncwerkError:
        repos = []
    return repos
...
def get_org_repos(org_id, start, limit):
    ...
    """
    try:
        repos = syncwserv_threaded_rpc.get_org_repo_list(org_id, start, limit)
    except RpcsyncwerkError:
        repos = []
...
def get_org_id_by_repo_id(repo_id):
    ...
    """
    try:
        org_id = syncwserv_threaded_rpc.get_org_id_by_repo_id(repo_id)
    except RpcsyncwerkError:
        org_id = -1
    return org_id
...
def get_org_repo_owner(repo_id):
    ...
    """
    try:
        owner = syncwserv_threaded_rpc.get_org_repo_owner(repo_id)
    except RpcsyncwerkError:
        owner = None
    return owner
...
def get_commits(repo_id, offset, limit):
    """Get commit lists."""
    try:
        ret = syncwserv_threaded_rpc.get_commit_list(repo_id, offset, limit)
    except RpcsyncwerkError:
        ret = None
    return ret
...
def get_branches(repo_id):
    ...
    return syncwserv_threaded_rpc.branch_gets(repo_id)
...
def get_group_repos_by_owner(user):
    ...
    """
    try:
        ret = syncwserv_threaded_rpc.get_group_repos_by_owner(user)
    except RpcsyncwerkError:
        ret = []
    return ret
...
def get_group_repos(group_id, user):
    ...
    for repo_id in repoid_list:
        ...
        repo.owner = syncwserv_threaded_rpc.get_group_repo_owner(repo_id)
...
def get_org_group_repoids(org_id, group_id):
    try:
        repo_ids = syncwserv_threaded_rpc.get_org_group_repoids(org_id, group_id)
    except RpcsyncwerkError:
        repo_ids = ''
...
def get_org_group_repos(org_id, group_id, user):
    ...
    for repo_id in repoid_list:
        ...
        repo.owner = syncwserv_threaded_rpc.get_org_group_repo_owner(org_id,
                                                                    group_id,
                                                                    repo_id)
...
def get_org_groups_by_repo(org_id, repo_id):
    try:
        group_ids = syncwserv_threaded_rpc.get_org_groups_by_repo(org_id,
                                                                 repo_id)
    except RpcsyncwerkError:
        group_ids = ''
    if not group_ids:
...
def list_inner_pub_repos_by_owner(user):
    ...
    """
    try:
        ret = syncwserv_threaded_rpc.list_inner_pub_repos_by_owner(user)
    except RpcsyncwerkError:
        ret = []
    return ret
...
def list_inner_pub_repos(username):
    ...
    """
    try:
        shared_repos = syncwserv_threaded_rpc.list_inner_pub_repos()
    except:
        shared_repos = []
...
def count_inner_pub_repos():
    try:
        ret = syncwserv_threaded_rpc.count_inner_pub_repos()
    except RpcsyncwerkError:
        ret = -1
    return 0 if ret < 0 else ret
...
def is_inner_pub_repo(repo_id):
    ...
    """
    try:
        ret = syncwserv_threaded_rpc.is_inner_pub_repo(repo_id)
    except RpcsyncwerkError:
        ret = 0
...
def unset_inner_pub_repo(repo_id):
    syncwserv_threaded_rpc.unset_inner_pub_repo(repo_id)
...
def list_org_inner_pub_repos(org_id, username, start=None, limit=None):
    ...
    """
    try:
        shared_repos = syncwserv_threaded_rpc.list_org_inner_pub_repos(org_id)
    except RpcsyncwerkError:
        shared_repos = []
...
def check_permission(repo_id, user):
    ...
    """
    try:
        ret = syncwserv_threaded_rpc.check_permission(repo_id, user)
    except RpcsyncwerkError:
        ret = None
    return ret
...
def is_personal_repo(repo_id):
    ...
    """
    try:
        owner = syncwserv_threaded_rpc.get_repo_owner(repo_id)
    except RpcsyncwerkError:
        owner = ''
...
def list_share_repos(user, share_type, start, limit):
    try:
        ret = syncwserv_threaded_rpc.list_share_repos(user, share_type,
                                                     start, limit)
    except RpcsyncwerkError:
        ret = []
    return ret
...
def remove_share(repo_id, from_user, to_user):
    syncwserv_threaded_rpc.remove_share(repo_id, from_user, to_user)

def unshare_group_repo(repo_id, group_id, from_user):
    return syncwserv_threaded_rpc.group_unshare_repo(repo_id, int(group_id),
                                                    from_user)
...
def list_org_shared_repos(org_id, user, user_type, start, limit):
    ...
    """
    try:
        share_repos = syncwserv_threaded_rpc.list_org_share_repos(org_id,
                                                                 user, user_type,
                                                                 start, limit)
    except RpcsyncwerkError:
        share_repos = []
...
def list_dir_by_path(repo_id, commit_id, path):
    try:
        ret = syncwserv_threaded_rpc.list_dir_by_path(repo_id, commit_id, path)
    except RpcsyncwerkError:
        ret = None
    return ret
...
def post_empty_file(repo_id, parent_dir, file_name, user):
    ...
    """
    try:
        ret = syncwserv_threaded_rpc.post_empty_file(repo_id, parent_dir,
                                              file_name, user)
    except RpcsyncwerkError, e:
        logger.error(e)
        ret = -1
...
def del_file(repo_id, parent_dir, file_name, user):
    ...
    """
    try:
        ret = syncwserv_threaded_rpc.del_file(repo_id, parent_dir,
                                             file_name, user)
    except RpcsyncwerkError, e:
        logger.error(e)
        ret = -1
...
def is_valid_filename(file_or_dir):
    ...
    """
    try:
        ret = syncwserv_threaded_rpc.is_valid_filename('', file_or_dir)
    except RpcsyncwerkError:
        ret = 0
...
def get_file_size(store_id, version, file_id):
    try:
        fs = syncwserv_threaded_rpc.get_file_size(store_id, version, file_id)
    except RpcsyncwerkError, e:
        fs = 0
    return fs
...
def get_file_id_by_path(repo_id, path):
    try:
        ret = syncwserv_threaded_rpc.get_file_id_by_path(repo_id, path)
    except RpcsyncwerkError, e:
        ret = ''
    return ret
...
def check_quota(repo_id, delta=0):
    try:
        ret = syncwserv_threaded_rpc.check_quota(repo_id, delta)
    except RpcsyncwerkError, e:
        logger.error(e)
        ret = -1
    return ret
...
def get_user_quota(user):
    try:
        ret = syncwserv_threaded_rpc.get_user_quota(user)
    except RpcsyncwerkError, e:
        logger.error(e)
        ret = 0
    return ret
...
def get_user_quota_usage(user):
    try:
        ret = syncwserv_threaded_rpc.get_user_quota_usage(user)
    except RpcsyncwerkError, e:
        logger.error(e)
        ret = 0
    return ret
...
def get_user_share_usage(user):
    try:
        ret = syncwserv_threaded_rpc.get_user_share_usage(user)
    except RpcsyncwerkError, e:
        logger.error(e)
        ret = 0
    return ret
...
def web_get_access_token(repo_id, obj_id, op, username, use_onetime=1):
    try:
        ret = syncwserv_rpc.web_get_access_token(repo_id, obj_id, op, username, use_onetime)
    except RpcsyncwerkError, e:
        ret = ''
    return ret
...
def unset_repo_passwd(repo_id, user):
    ...
    """
    try:
        ret = syncwserv_threaded_rpc.unset_passwd(repo_id, user)
    except RpcsyncwerkError, e:
        ret = -1
    return ret
...
def is_passwd_set(repo_id, user):
    try:
        ret = syncwserv_rpc.is_passwd_set(repo_id, user)
    except RpcsyncwerkError, e:
        ret = -1
    return True if ret == 1 else False
...
def get_repo_history_limit(repo_id):
    try:
        ret = syncwserv_threaded_rpc.get_repo_history_limit(repo_id)
    except RpcsyncwerkError, e:
        ret = -1
    return ret
...
def set_repo_history_limit(repo_id, days):
    try:
        ret = syncwserv_threaded_rpc.set_repo_history_limit(repo_id, days)
    except RpcsyncwerkError, e:
        ret = -1
    return ret
